...
* * 
* * Description: 
* * 1. Defining the class boostudp, for receiving and sending udp data.
* *
* * History: 
* * Xuekui Zhu    16/09/20    1.0.0    build the boostudp class.
* ****************************************************************/
#ifndef BOOST_UDP_H
#define BOOST_UDP_H
...
class Boost_UDP
{
    ...
    int receive_data(unsigned char buf[])
    {
        ...
        int ret = udp_sock.receive_from(boost::asio::buffer(buf,RECVSIZE),send_endpoint);
        return ret;
    }
